...
type ToolCallPartCtor = new (...args: any[]) => {
  callId?: string;
  name?: string;
...
export class VsCodeLmProvider extends LLMProvider {
    ...
    this.configurationDisposable = vscode.workspace.onDidChangeConfiguration(
      (event) => {
        if (
          ...
          Logger.info(
            "VS Code LM configuration changed; resetting cached model."
          );
          ...
          this.refreshModels().catch((error) => {
            Logger.warn(
              `VS Code LM model refresh failed: ${
  ...
  async initialize(): Promise<void> {
    ...
    await this.refreshModels().catch((error) => {
      Logger.warn(
        `VS Code LM initial model discovery failed: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
    });
    ...
    try {
      ...
      if (workspaceRoot) {
        ...
        if (!fs.existsSync(superdesignDir)) {
          ...
          Logger.info(
            `Created .superdesign directory for VS Code LM provider: ${superdesignDir}`
          );
        }
        ...
        if (!fs.existsSync(tempDir)) {
          ...
          Logger.info(
            `Created temporary directory for VS Code LM provider: ${tempDir}`
          );
        }
      ...
      Logger.error(
        `Failed to setup working directory for VS Code LM provider: ${error}`
      );
  ...
  private async refreshModels(): Promise<void> {
    ...
    if (!lmApi || typeof lmApi.selectChatModels !== "function") {
      Logger.warn(
        "VS Code language model API is unavailable. Ensure VS Code 1.95+ with LM API enabled."
      );
      this.availableModels = [];
      return;
    }
    ...
    try {
      ...
      if (this.availableModels.length === 0) {
        Logger.warn(
          "No VS Code language models detected. Sign in to GitHub Copilot or install another LM provider."
        );
      }
    } catch (error) {
      Logger.warn(
        `Failed to enumerate VS Code language models: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
      this.availableModels = [];
  ...
  private selectModelFromCollection(
    models: VsCodeChatModel[],
    modelId: string
  ): VsCodeChatModel | undefined {
    ...
    const direct = models.find(
      (model) => this.buildModelKey(model).toLowerCase() === normalizedTarget
    );
    if (direct) {
      return direct;
    }
  ...
  private async ensureClient(
    ...
    if (!lmApi || typeof lmApi.selectChatModels !== "function") {
      const reason =
        "VS Code language model API is unavailable. Please restart VS Code or enable the LM preview.";
      Logger.warn(reason);
    ...
    try {
      const models: VsCodeChatModel[] = await lmApi.selectChatModels(
        selector as any
      );
      this.availableModels = Array.isArray(models) ? models : [];

      const resolved = this.selectModelFromCollection(
        this.availableModels,
        modelId
      );
      if (resolved) {
      ...
      const reason =
        "No VS Code language models match the selected criteria. Install and sign in to GitHub Copilot or update your Superdesign VS Code LM settings.";
      Logger.warn(reason);
      const fallbackClient = this.createFallbackClient(reason);
      this.client = fallbackClient;
      this.clientSelectorKey = selectorKey;
      ...
      const reason = `Failed to select VS Code language model: ${
        error instanceof Error ? error.message : String(error)
      }`;
      Logger.warn(reason);
      const fallbackClient = this.createFallbackClient(reason);
  ...
  private loadSystemPrompt(customPrompt?: string): string {
    ...
    try {
      ...
      Logger.warn(
        `Failed to load system prompt for VS Code LM provider: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
    }
...
- **Responsive design** that looks perfect on mobile, tablet and desktop.`;
  ...
  async query(
    ...
    const { client, degraded, reason } = await this.ensureClient(
      selector,
      modelId
    );
    ...
    if (!client || typeof client.sendRequest !== "function") {
      throw new Error(
        "Selected VS Code LM model is unavailable. Please ensure GitHub Copilot (or another provider) is installed and signed in."
      );
    }

    const systemPrompt = this.loadSystemPrompt(options?.customSystemPrompt);
    const structuredMessages = Array.isArray((options as any)?.messages)
      ? (options as any).messages
      : undefined;
    const { messages, textForCounting } = this.buildVsCodeMessages(
      systemPrompt,
      prompt,
      structuredMessages
    );
    ...
    const metadata = degraded
      ? { degraded: true, diagnostic: reason }
      : undefined;
    ...
    try {
      const response = await client.sendRequest(
        messages,
        {
          justification: `Superdesign would like to use '${
            client.name ?? "language model"
          }' from '${client.vendor ?? "unknown"}'. Click 'Allow' to proceed.`,
        },
        cancellationToken
      );
      ...
      if (!response || !this.isAsyncIterable(response.stream)) {
        const fallbackText =
          reason ??
          "VS Code language model did not provide a streaming response. Please verify your Copilot access.";
        ...
        for await (const rawChunk of response.stream as AsyncIterable<unknown>) {
          ...
          if (textPartCtor && rawChunk instanceof textPartCtor) {
            ...
            if (!toolChunk.callId || !toolChunk.name) {
              Logger.warn(
                "VS Code LM provider received incomplete tool call chunk; skipping."
              );
              continue;
            }
            ...
            Logger.debug(
              `VS Code LM provider received unsupported chunk type: ${JSON.stringify(
                rawChunk
              )}`
            );
          }
      ...
      Logger.info(
        `VS Code LM usage (est): input ~${inputTokens} tokens, output ~${outputTokens} tokens.`
      );
  ...
  async waitForInitialization(): Promise<boolean> {
    try {
      ...
      Logger.error(
        `VS Code LM provider initialization failed while waiting: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
      return false;
  ...
  async refreshConfiguration(): Promise<boolean> {
    try {
      ...
      Logger.error(
        `Failed to refresh VS Code LM configuration: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
      return false;
  ...
  private buildVsCodeMessages(
    ...
    if (
      !lmChatMessage ||
      typeof lmChatMessage.User !== "function" ||
      typeof lmChatMessage.Assistant !== "function"
    ) {
      throw new Error(
        "VS Code language model API is unavailable. Please ensure you are using VS Code 1.95+."
      );
    }
  ...
  private transformMessageContent(
    ...
    if (!Array.isArray(content)) {
      const serialized =
        typeof content === "undefined" ? "" : JSON.stringify(content);
    ...
    if (role === "user") {
      for (const part of content) {
        if (part?.type === "tool-result") {
          ...
          const resultPart = this.createToolResultPart(
            toolResultCtor,
            callId,
            segments,
            textCtor
          ...
          pushText(
            `[Image (${part.source?.type ?? "unknown"}): ${
              part.source?.media_type ?? "unknown"
            } not supported by VS Code LM API]`
          );
      ...
      for (const part of content) {
        if (part?.type === "tool-call" || part?.type === "tool_use") {
          const callId =
            part.toolCallId ?? part.tool_use_id ?? part.id ?? "tool-call";
          const name = part.toolName ?? part.name ?? "tool";
          const input = part.args ?? part.input ?? {};
          const toolCallPart = this.createToolCallPart(
            toolCallCtor,
            callId,
            name,
            input
  ...
  private createToolCallPart(
    ...
    try {
      ...
      Logger.warn(
        `Failed to instantiate LanguageModelToolCallPart: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
      return null;
  ...
  private createToolResultPart(
    ...
    try {
      ...
      Logger.warn(
        `Failed to instantiate LanguageModelToolResultPart: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
      return null;
